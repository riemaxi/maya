#ifndef CONFIG_HPP
#define CONFIG_HPP

#include <string>
#include <map>

namespace Config {

    struct Credential {
        std::string accesskey = "SAMUEL";
        std::string password = "000000";
        std::string address = "samuel.maya.4da";
        // Context is empty in JS, represented here as a map
        std::map<std::string, std::string> context;
    };

    struct Peers {
        std::string pingpong = "pingpong.maya.4da";
    };

    struct Security {
        // Using R"(...)" for raw string literal to preserve formatting and newlines
        std::string ca = R"(-----BEGIN CERTIFICATE-----

EKQqNgEi8YLsgqSxKRJ4xJfjD/Eg13fAaUP6vfuc9fHj4TUSbtiMpLV4
-----END CERTIFICATE-----)";
    };

    struct SystemConfig {
        std::string host_0 = "wss://194.163.178.202:30000";
        std::string host_1 = "wss://154.12.245.142:30000";
        std::string host   = "wss://103.177.249.118:30050";
        
        Credential credential;
        Peers peers;
        Security security;
    };

    // Global instance of the configuration
    static const SystemConfig system;
}

#endif // CONFIG_HPP